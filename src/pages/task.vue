<template>
  ...
  <img class="resource-banner" src="../assets/banner6.png">
  <div class="all-content">
       <div class="form-search">
          <el-form ref="form" :model="form" :inline="true">
              <el-form-item>
                <el-input v-model="form.search" placeholder="请输入实训名称"></el-input>
              </el-form-item>
              <el-form-item>
          ...
          <div class="search-right">
            <router-link :to="{path:'/taskmanage'}">任务管理</router-link>
          </div>
       </div>
  </div>
  <div class="all-content">
		...
		<div class="resource-li">
			<div class="resource-title">
				<img src="../assets/reicon3.png">
				<h1 v-html="datatitle[id]"></h1>
				<img src="../assets/reicon4.png">
				<a @click="taskList" class="resource-list">{{ isdata }}</a>
			</div>
          ...
          stripe
					...
					<template slot-scope="scope">
						...
						size="mini"
						class="resour-eye"
						@click="taskEye(scope.$index, id)">查看</el-button>
						<el-button
						size="mini"
						class="resour-an"
						@click="taskAn(scope.$index)">安排</el-button>
					</template>
					</el-table-column>
				    </el-table>
				</div>
			...
			<div class="material-li" v-if="islist">
              ...
              <img :src="item.imgurl" class="materialImg">
					...
					<p>{{ item.name }}</p>
					<span class="resoue-span">实训简介：{{ item.content }}</span>
					<div class="resour-button">
						<a class="resour-eye" title="查看" @click="taskEye(item.id,id)">查看</a>
						<a class="resour-an" title="安排" @click="taskAn(index)">安排</a>
					</div>
				</div>
...
export default {
  ...
  methods: {
	...
	taskEye(index,id){
		this.$router.push({path:'/taskDetails/'+index,
			query:{
			   taskid : id
			}
		 })
